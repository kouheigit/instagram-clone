      ...
      ? [
          {
            ...
            onClick: () => {
              setShowMenu(false);
              if (confirm("この投稿を削除しますか？")) handleDeletePost();
            },
          },
          {
            label: "編集",
            variant: "bold",
            onClick: () => { showToast("編集機能は準備中です", "success"); setShowMenu(false); },
          },
          {
            label: "いいね数を非表示",
            onClick: () => { showToast("この機能は準備中です", "success"); setShowMenu(false); },
          },
          {
            label: "コメントをオフにする",
            onClick: () => { showToast("この機能は準備中です", "success"); setShowMenu(false); },
      ...
      : [
          {
            label: "報告する",
            variant: "danger",
            onClick: () => { showToast("報告を受け付けました", "success"); setShowMenu(false); },
          },
          {
            label: "リンクをコピー",
            onClick: () => {
          ...
          {
            label: "このアカウントについて",
            onClick: () => { setShowMenu(false); router.push(`/profile/${author?.username ?? ""}`); },
          },
          {
            label: "キャンセル",
            variant: "cancel",
  ...
  return (
    <>
      ...
      <PostMenuModal
        ...
        items={commentDeleteMenuItems}
      />

      {/* ===== 暗いオーバーレイ背景 ===== */}
      <div className="fixed inset-0 bg-[#5c6572] z-30 flex items-center justify-center">

        {/* 閉じるボタン（右上） */}
        <button
          onClick={() => router.back()}
          className="absolute top-4 right-4 text-white hover:text-white/70 transition-colors z-50"
          aria-label="閉じる"
        >
        ...
        {/* 前の投稿ナビ（モーダル外左） */}
        <button
          className="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:text-white/70 transition-colors z-50"
          onClick={() => router.back()}
          aria-label="戻る"
        >
        ...
        {/* 次の投稿ナビ（モーダル外右、将来用） */}
        <div className="absolute right-4 top-1/2 -translate-y-1/2 w-10" />

        {/* ===== モーダル本体 ===== */}
        <div
          className="relative flex bg-white overflow-hidden z-40"
        ...
        >
                  ...
                  {post.media_files.map((_, i) => (
                    <div
                      ...
                      className={`w-1.5 h-1.5 rounded-full transition-colors ${
                        i === imgIdx ? "bg-[#0095f6]" : "bg-white/50"
                      }`}
                    />
                  ))}
                </div>
          ...
          {/* ---- 右: 情報パネル ---- */}
          <div className="flex flex-col flex-1 min-w-0 border-l border-[#dbdbdb]">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
              ...
              {post.caption && (
                <div className="flex gap-3">
                  ...
                  <div className="text-sm flex-1 min-w-0">
                    <span>
                      <Link href={`/profile/${author?.username ?? ""}`} className="font-semibold mr-1.5">
                        {author?.username}
                      </Link>
              ...
              {comments.map((c) => {
                ...
                const isOwnComment = me?.user_id === c.user_id;
                return (
                  <div key={c.comment_id}>
                    <div className="flex gap-3 group/comment">
                      <Link href={`/profile/${cAuthor?.username ?? ""}`} className="flex-shrink-0 mt-0.5">
                        <Avatar src={cAuthor?.profile_img} username={cAuthor?.username} size={32} />
                      </Link>
                      <div className="text-sm flex-1 min-w-0">
                        <span>
                          <Link href={`/profile/${cAuthor?.username ?? ""}`} className="font-semibold mr-1.5">
                            {cAuthor?.username ?? "ユーザー"}
                          </Link>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <button
                            onClick={() => handleReplyClick(c, cAuthor)}
                            className="text-[#8e8e8e] text-xs font-semibold hover:text-[#262626]"
                          >
                            返信する
                          </button>
                          {c.like_count > 0 && (
                            <span className="text-[#8e8e8e] text-xs">{c.like_count}件のいいね</span>
                          )}
                        </div>
                      </div>
                      {isOwnComment && (
                        <button
                          ...
                          className="flex-shrink-0 self-start mt-1 text-[#8e8e8e] opacity-0 group-hover/comment:opacity-100 transition-opacity hover:text-[#262626]"
                          aria-label="コメントメニュー"
                        >
                    ...
                    {(c.replies ?? []).length > 0 && (
                      <div className="ml-11 mt-2 space-y-3">
                        {(c.replies ?? []).map((reply) => {
                          ...
                          const isOwnReply = me?.user_id === reply.user_id;
                          return (
                            <div key={reply.comment_id} className="flex gap-2 group/reply">
                              ...
                              <Link href={`/profile/${rAuthor?.username ?? ""}`} className="flex-shrink-0">
                                <Avatar src={rAuthor?.profile_img} username={rAuthor?.username} size={24} />
                              </Link>
                              <div className="text-sm flex-1 min-w-0">
                                <span>
                                  <Link href={`/profile/${rAuthor?.username ?? ""}`} className="font-semibold mr-1.5 text-xs">
                                    {rAuthor?.username ?? "ユーザー"}
                                  </Link>
                              ...
                              {isOwnReply && (
                                <button
                                  ...
                                  className="flex-shrink-0 self-start text-[#8e8e8e] opacity-0 group-hover/reply:opacity-100 transition-opacity hover:text-[#262626]"
                                  aria-label="返信メニュー"
                                >
            ...
            <div className="border-t border-[#dbdbdb] flex-shrink-0">
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    <Heart
                      size={24}
                      strokeWidth={1.5}
                      className={liked ? "fill-[#ed4956] stroke-[#ed4956]" : "stroke-[#262626]"}
                    />
                  </button>
                  <button
                    aria-label="コメント"
              ...
              <div className="px-4 pb-1">
                {likeCount > 0 ? (
                  ...
                  <p className="text-sm">
                    <span className="font-semibold">いいね！</span>した人はまだいません
                  </p>
                )}
                <p className="text-[#8e8e8e] text-[10px] uppercase mt-0.5">
              ...
              <form
                ...
                className="border-t border-[#dbdbdb] px-4 py-2"
              >
                {replyTo && (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-[#0095f6] font-semibold text-sm disabled:opacity-40 transition-opacity"
                  >
                    投稿する
                  </button>
                </div>
